...
- **All routes are wired:** every sidebar link and form target has a handler, and the student/teacher pages are fed real data (courses, quizzes, submissions, study groups, wallet). Study groups support create/join/leave (`student_handlers.go`); register honours `?role=student|teacher`; course-material PDFs are enrollment-gated (never served raw from `/static/uploads`).
